...
contains
   ...
   pure module function binary_cross_entropy_eval(true, predicted) result(res)
      ...
      real, intent(in) :: predicted(:)
      real :: res
      ! res = -sum(true * log(predicted) + (1.0 - true) * log(1.0 - predicted))
      res = sum((1.0 - true) * predicted + log(1.0 + exp(-predicted)))
   ...
   pure module function binary_cross_entropy_derivative(true, predicted) result(res)
      ...
      real, intent(in) :: predicted(:)
      real :: res(size(true))
      ! res = -true / predicted + (1.0 - true) / (1.0 - predicted)
      res = (1.0 - true) - 1.0 / (1.0 + exp(predicted))
